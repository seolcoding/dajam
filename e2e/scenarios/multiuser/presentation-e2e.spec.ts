...
test.describe('호스트 인증 및 세션 생성', () => {
  test.describe.configure({ mode: 'serial' });

  test('호스트가 로그인하여 대시보드 접근', async ({ page }) => {
    // 대시보드로 이동
  ...
  test('호스트가 audience-engage 세션 생성', async ({ page }) => {
...
test.describe('슬라이드 임베드', () => {
  ...
  test('Google Slides 공개 URL 임베드', async ({ page }) => {
  ...
  test('PDF 파일 업로드 및 표시', async ({ page }) => {
    // 파일 업로드 input 찾기
...
test.describe('멀티 브라우저 동시 접속', () => {
  test.setTimeout(180000); // 3분 타임아웃

  test('호스트 + 4명 참여자 동시 세션 참여', async ({ browser }) => {
...
test.describe('실시간 인터랙션', () => {
  test.setTimeout(120000);

  test('참여자들의 Q&A 질문이 호스트에게 실시간 전달', async ({ browser }) => {
    const allContexts: BrowserContext[] = [];
  ...
  test('이모지 리액션 실시간 전송', async ({ browser }) => {
    const allContexts: BrowserContext[] = [];
...
test.describe('호스트 슬라이드 제어', () => {
  test.setTimeout(120000);

  test('호스트가 슬라이드 넘기면 참여자도 동기화', async ({ browser }) => {
    const allContexts: BrowserContext[] = [];
...
test.describe('결과 통합', () => {
  test('세션 종료 시 모든 인터랙션 데이터 집계', async ({ browser }) => {
    const hostContext = await browser.newContext();
    const hostPage = await hostContext.newPage();
...
test.describe('디바이스 혼합 반응형', () => {
  test('다양한 디바이스에서 동시 접속 시 UI 정상 표시', async ({ browser }) => {
    const devices = [
